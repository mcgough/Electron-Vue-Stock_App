...
const mutations = {
  SET_SYMBOL_TIME_SERIES(state, params) {
    ...
    if (utils.hasOwnProp(response, "Meta Data")) {
      ...
      const dataPoints = Object.keys(series).map(data =>
        utils.cleanOrderedProps(series[data])
      );
  ...
  SET_SYMBOL_INDICATOR(state, params) {
    ...
    if (utils.hasOwnProp(response, "Meta Data")) {
      ...
      const {
        "Meta Data": meta,
        [`Technical Analysis: ${type}`]: technical
      } = response;
      const dataPoints = Object.keys(technical).map(
        data => technical[data][type]
      );
